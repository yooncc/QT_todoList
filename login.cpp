    ...
    : QWidget{parent}
{
    ...
    pwField->setGeometry(QRect(margin,(idField->y())+(idField->height())+margin,viewWidth-(margin*2),30));

    loginBtn = util.makePushButton(this,"Log in","",8,false,"");
    loginBtn->setGeometry(QRect(margin,pwField->y()+pwField->height()+margin,(viewWidth-(margin*3))/2,30));
    connect(loginBtn,SIGNAL(clicked()),this,SLOT(startLogin()));

    signBtn = util.makePushButton(this,"Sign up","",8,false,"");
    signBtn->setGeometry(QRect(loginBtn->x()+loginBtn->width()+margin,pwField->y()+pwField->height()+margin,(viewWidth-(margin*3))/2,30));
    connect(signBtn,SIGNAL(clicked()),this,SLOT(startSignUp()));

    findPwBtn = util.makePushButton(this,"비밀번호 찾기","",8,false,"");
    findPwBtn->setGeometry(QRect(margin,loginBtn->y()+loginBtn->height()+margin,viewWidth-(margin*2),30));
    connect(findPwBtn,SIGNAL(clicked(bool)),this,SLOT(startFindPw()));

    helpBtn = util.makePushButton(this,"도움말","",8,false,"");
    helpBtn->setGeometry(QRect(margin,findPwBtn->y()+findPwBtn->height()+margin,viewWidth-(margin*2),30));
